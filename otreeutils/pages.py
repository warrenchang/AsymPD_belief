...
class ExtendedPage(Page):
    ...
    def get_context_data(self, **kwargs):
        ...
        debug = self.session.config.get('debug',self.debug)
        ctx.update({
            ...
            'debug': int(debug),   # allows to retrieve a debug state in the templates
            'debug_fill_forms_randomly': int(debug and self.debug_fill_forms_randomly)
        })
...
class UnderstandingQuestionsPage(ExtendedPage):
    ...
    debug = APPS_DEBUG
    set_correct_answers = False         # useful for skipping pages during development
    ...
    def vars_for_template(self):
        ...
        quiz_attempts = self.session.config.get('quiz_attempts',0)
        ...
        v = {
            ...
            'set_correct_answers': str(self.set_correct_answers and self.session.config.get('debug',self.debug)).lower(),
        }
